...
namespace SiaNet.Model.Initializers
{
    ...
    public class Normal : InitializerBase
    {
        ...
        public Normal(double scale, int outputRank) : this(scale)
        {
        ...
        public Normal(double scale, int outputRank, int filterRank) : this(scale, outputRank)
        {
        ...
        public Normal(double scale, int outputRank, int filterRank, uint seed) : this(scale, outputRank, filterRank)
        {
            Seed = seed;
        }
        ...
        internal override CNTKDictionary ToDictionary()
        {
            if (HasOutputRank)
            {
                if (HasFilterRank)
                {
                    if (HasSeed)
                    {
                        return CNTKLib.NormalInitializer(Scale, OutputRank, FilterRank, Seed);
                    }

                    return CNTKLib.NormalInitializer(Scale, OutputRank, FilterRank);
                }
